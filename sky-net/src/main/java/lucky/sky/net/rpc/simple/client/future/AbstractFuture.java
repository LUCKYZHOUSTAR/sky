...
import lucky.sky.util.lang.UnsafeUtil;
...
public abstract class AbstractFuture<V> {
  ...
  /**
   * 内部状态转换过程: NEW -> COMPLETING -> NORMAL          // 正常完成 NEW -> COMPLETING -> EXCEPTIONAL     //
   * 出现异常
   */
  ...
  protected void setException(Object t) {
    if (UNSAFE.compareAndSwapInt(this, stateOffset, NEW, COMPLETING)) {
      outcome = t;
      // putOrderedInt在JIT后会通过intrinsic优化掉StoreLoad屏障, 不保证可见性
  ...
  /**
   * 1. 唤醒并移除Treiber Stack中所有等待线程 2. 调用钩子函数done()
   */
